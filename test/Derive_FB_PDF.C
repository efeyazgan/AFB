...
void tree1r()
{
  ...
  */
  ...
  int n_mstw = 41;
  int n_nnpdf = 101;
  float ct10Weights[n_ct];
  float mstw2008Weights[n_mstw];
  float nnpdf21Weights[n_nnpdf];
  ...
  TH1D *hMeasCos_M_Y_Forward_PDF[nb_Y][nPDF];
  TH1D *hMeasCos_M_Y_Backward_PDF[nb_Y][nPDF];
...
////////////////////////////////////////
  ...
  for (int j=0;j<nb_Y;j++){
    for (int k=0;k<nPDF;k++){
      sprintf(name_h,"hMeasCos_Forward_PDF_Y_%i_W_%i",j,k);
      hMeasCos_M_Y_Forward_PDF[j][k] = new TH1D(name_h,name_h,nb, xAxis_AFB);
      sprintf(name_h,"hMeasCos_Backward_PDF_Y_%i_W_%i",j,k);
      hMeasCos_M_Y_Backward_PDF[j][k] = new TH1D(name_h,name_h,nb, xAxis_AFB);
    }
    ...
    sprintf(name_h,"Inv_Response_UnDetector_Forward_PDF_Y_%i_",j);
    Inv_Response_UnDetector_Forward_PDF[j] = new TH2D(name_h,name_h,nb, xAxis_AFB,nb, xAxis_AFB);
    sprintf(name_h,"Inv_Response_UnDetector_Backward_PDF_Y_%i_",j);
    Inv_Response_UnDetector_Backward_PDF[j] = new TH2D(name_h,name_h,nb, xAxis_AFB,nb, xAxis_AFB);

    sprintf(name_h,"Inv_Response_NoFsr_Forward_PDF_Y_%i_",j);
    Inv_Response_NoFsr_Forward_PDF[j] = new TH2D(name_h,name_h,nb, xAxis_AFB,nb, xAxis_AFB);
    sprintf(name_h,"Inv_Response_NoFsr_Backward_PDF_Y_%i_",j);
    Inv_Response_NoFsr_Backward_PDF[j] = new TH2D(name_h,name_h,nb, xAxis_AFB,nb, xAxis_AFB);
...
*/
    ...
    if (!realdata){
	  ...
	  if (fabs(Ymuon) > Y_bin_limits[j] && fabs(Ymuon) < Y_bin_limits[j+1]){ 
	    for (int pdfindex=0; pdfindex < nPDF; pdfindex++){
	      if (costhetaCSreco__RECO > 0){ 
		//cout<<j<<"   "<<pdfindex<<"    "<<MZ__RECO<<"   "<<ct10Weights[pdfindex]<<endl;
		if (pdf_set == 0) hMeasCos_M_Y_Forward_PDF[j][pdfindex]->Fill(MZ__RECO,ct10Weights[pdfindex]);
		if (pdf_set == 1) hMeasCos_M_Y_Forward_PDF[j][pdfindex]->Fill(MZ__RECO,nnpdf21Weights[pdfindex]);
		if (pdf_set == 2) hMeasCos_M_Y_Forward_PDF[j][pdfindex]->Fill(MZ__RECO,mstw2008Weights[pdfindex]);
		if (pdf_set == 3) hMeasCos_M_Y_Forward_PDF[j][pdfindex]->Fill(MZ__RECO,mstw2008Weights[pdfindex]);
	      }
	      if (costhetaCSreco__RECO < 0 && costhetaCSreco__RECO >= -1.){
		if (pdf_set == 0) hMeasCos_M_Y_Backward_PDF[j][pdfindex]->Fill(MZ__RECO,ct10Weights[pdfindex]);
		if (pdf_set == 1) hMeasCos_M_Y_Backward_PDF[j][pdfindex]->Fill(MZ__RECO,nnpdf21Weights[pdfindex]);
		if (pdf_set == 2) hMeasCos_M_Y_Backward_PDF[j][pdfindex]->Fill(MZ__RECO,mstw2008Weights[pdfindex]);
		if (pdf_set == 3) hMeasCos_M_Y_Backward_PDF[j][pdfindex]->Fill(MZ__RECO,mstw2008Weights[pdfindex]);
	      }
	    }
	    if (gen_st1_select){
  ...
  for (int i=0;i<nb;i++){
	...
	if (test_bb_NoFsr_3[k] != 1) cout<<"diagonal for the inv x resp for backward fsr matrix  "<< i<<"   "<<j<<"   "<<test_bb_NoFsr_3[k]<<endl;
      ...
      Inv_Response_UnDetector_Forward_PDF[0]->SetBinContent(i+1,j+1,inv_ff_0[k]);
      Inv_Response_UnDetector_Forward_PDF[1]->SetBinContent(i+1,j+1,inv_ff_1[k]);
      Inv_Response_UnDetector_Forward_PDF[2]->SetBinContent(i+1,j+1,inv_ff_2[k]);
      Inv_Response_UnDetector_Forward_PDF[3]->SetBinContent(i+1,j+1,inv_ff_3[k]);

      Inv_Response_NoFsr_Forward_PDF[0]->SetBinContent(i+1,j+1,inv_ff_NoFsr_0[k]);
      Inv_Response_NoFsr_Forward_PDF[1]->SetBinContent(i+1,j+1,inv_ff_NoFsr_1[k]);
      Inv_Response_NoFsr_Forward_PDF[2]->SetBinContent(i+1,j+1,inv_ff_NoFsr_2[k]);
      Inv_Response_NoFsr_Forward_PDF[3]->SetBinContent(i+1,j+1,inv_ff_NoFsr_3[k]);

      Inv_Response_UnDetector_Backward_PDF[0]->SetBinContent(i+1,j+1,inv_bb_0[k]);
      Inv_Response_UnDetector_Backward_PDF[1]->SetBinContent(i+1,j+1,inv_bb_1[k]);
      Inv_Response_UnDetector_Backward_PDF[2]->SetBinContent(i+1,j+1,inv_bb_2[k]);
      Inv_Response_UnDetector_Backward_PDF[3]->SetBinContent(i+1,j+1,inv_bb_3[k]);

      Inv_Response_NoFsr_Backward_PDF[0]->SetBinContent(i+1,j+1,inv_bb_NoFsr_0[k]);
      Inv_Response_NoFsr_Backward_PDF[1]->SetBinContent(i+1,j+1,inv_bb_NoFsr_1[k]);
      Inv_Response_NoFsr_Backward_PDF[2]->SetBinContent(i+1,j+1,inv_bb_NoFsr_2[k]);
      Inv_Response_NoFsr_Backward_PDF[3]->SetBinContent(i+1,j+1,inv_bb_NoFsr_3[k]);
    }
  }
    ...
    for (int j=0;j<nb_Y;j++){
      for (int k=0;k<nPDF;k++){
	sprintf(name_h,"MC_meas_Forward_PDF_%i_pdf_%i_set_%i",j,k,pdf_set);
	file_cov->WriteTObject(hMeasCos_M_Y_Forward_PDF[j][k],name_h);
	sprintf(name_h,"MC_meas_Backward_PDF_%i_pdf_%i_set_%i",j,k,pdf_set);
	file_cov->WriteTObject(hMeasCos_M_Y_Backward_PDF[j][k],name_h);
      }
      sprintf(name_h,"inv_response_undetector_forward_PDF_%i_set_%i",j,pdf_set);
      file_cov->WriteTObject(Inv_Response_UnDetector_Forward_PDF[j],name_h);
      sprintf(name_h,"inv_response_undetector_backward_PDF_%i_set_%i",j,pdf_set);
      file_cov->WriteTObject(Inv_Response_UnDetector_Backward_PDF[j],name_h);
	
      sprintf(name_h,"inv_response_nofsr_forward_PDF_%i_set_%i",j,pdf_set);
      file_cov->WriteTObject(Inv_Response_NoFsr_Forward_PDF[j],name_h);
      sprintf(name_h,"inv_response_nofsr_backward_PDF_%i_set_%i",j,pdf_set);
      file_cov->WriteTObject(Inv_Response_NoFsr_Backward_PDF[j],name_h);
    }
    //  file_cov->Write();
