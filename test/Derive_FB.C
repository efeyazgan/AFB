...
////////////////////////////////////////
  ...
  for (int j=0;j<nb_Y;j++){
    ...
    sprintf(name_h,"hMeasCos_Backward_FSR_UP_Y_%i_",j);
    hMeasCos_M_Y_Backward_FSR_UP[j] = new TH1D(name_h,name_h,nb, xAxis_AFB);

    sprintf(name_h,"hMeasCos_Forward_FSR_DOWN_Y_%i_",j);
    hMeasCos_M_Y_Forward_FSR_DOWN[j] = new TH1D(name_h,name_h,nb, xAxis_AFB);
    ...
    sprintf(name_h,"Response_NoFsr_Forward_Y_FSR_UP_%i_",j);
    Response_NoFsr_Forward_FSR_UP[j] = new TH2D(name_h,name_h,nb, xAxis_AFB,nb, xAxis_AFB);
    sprintf(name_h,"Response_NoFsr_Backward_Y_FSR_UP_%i_",j);
    Response_NoFsr_Backward_FSR_UP[j] = new TH2D(name_h,name_h,nb, xAxis_AFB,nb, xAxis_AFB);

    sprintf(name_h,"Response_NoFsr_Forward_Y_FSR_DOWN_%i_",j);
    Response_NoFsr_Forward_FSR_DOWN[j] = new TH2D(name_h,name_h,nb, xAxis_AFB,nb, xAxis_AFB);
    ...
    sprintf(name_h,"Inv_Response_NoFsr_Forward_Y_%i_",j);
    Inv_Response_NoFsr_Forward[j] = new TH2D(name_h,name_h,nb, xAxis_AFB,nb, xAxis_AFB);
    sprintf(name_h,"Inv_Response_NoFsr_Backward_Y_%i_",j);
    Inv_Response_NoFsr_Backward[j] = new TH2D(name_h,name_h,nb, xAxis_AFB,nb, xAxis_AFB);

    sprintf(name_h,"Inv_Response_NoFsr_Forward_Y_FSR_UP_%i_",j);
    Inv_Response_NoFsr_Forward_FSR_UP[j] = new TH2D(name_h,name_h,nb, xAxis_AFB,nb, xAxis_AFB);
    sprintf(name_h,"Inv_Response_NoFsr_Backward_FSR_UP_Y_%i_",j);
    Inv_Response_NoFsr_Backward_FSR_UP[j] = new TH2D(name_h,name_h,nb, xAxis_AFB,nb, xAxis_AFB);

    sprintf(name_h,"Inv_Response_NoFsr_Forward_Y_FSR_DOWN_%i_",j);
...
//cout<<"weight:     "<<weight<<endl;
	...
	*/
    ...
    if (!realdata){
	  ...
	  if (fabs(Ymuon) > Y_bin_limits[j] && fabs(Ymuon) < Y_bin_limits[j+1]){ 
		...
		Response_NoFsr_Forward_FSR_UP[j]->Fill(MZ, MZ__RECO,weight_up);
		Response_NoFsr_Forward_FSR_DOWN[j]->Fill(MZ, MZ__RECO,weight_down);
	      }
		...
		Response_NoFsr_Backward_FSR_UP[j]->Fill(MZ, MZ__RECO, weight_up);
		Response_NoFsr_Backward_FSR_DOWN[j]->Fill(MZ, MZ__RECO, weight_down);		
	      }
	    }
	  }
	}
  ...
  //fsr syst.
  double norm_factor_fsr_forward_FSR_UP[nb][nb_Y];
  double norm_factor_fsr_backward_FSR_UP[nb][nb_Y];
  ...
  for (int k=0;k<nb_Y;k++){
    for (int i=0;i<nb;i++){
      ...
      //fsr syst.
      norm_factor_fsr_forward_FSR_UP[i][k] = 0;
      norm_factor_fsr_backward_FSR_UP[i][k] = 0;
	...
	//fsr syst.
	norm_factor_fsr_forward_FSR_UP[i][k] += Response_NoFsr_Forward_FSR_UP[k]->GetBinContent(i+1,j+1);
	norm_factor_fsr_backward_FSR_UP[i][k] += Response_NoFsr_Backward_FSR_UP[k]->GetBinContent(i+1,j+1);
	...
	//
      ...
      //fsr syst.
      if (norm_factor_fsr_forward_FSR_UP[i][k] != 0.) for (int j=0;j<nb;j++) Response_NoFsr_Forward_FSR_UP[k]->SetBinContent(i+1,j+1,Response_NoFsr_Forward_FSR_UP[k]->GetBinContent(i+1,j+1)/norm_factor_fsr_forward_FSR_UP[i][k]);
      if (norm_factor_fsr_backward_FSR_UP[i][k] != 0.) for (int j=0;j<nb;j++) Response_NoFsr_Backward_FSR_UP[k]->SetBinContent(i+1,j+1,Response_NoFsr_Backward_FSR_UP[k]->GetBinContent(i+1,j+1)/norm_factor_fsr_backward_FSR_UP[i][k]);
  ...
  //fsr syst
  TMatrixD Rij_NoFsr_FF_FSR_UP_0(nb,nb);
  TMatrixD Rij_NoFsr_FF_FSR_UP_1(nb,nb);
  TMatrixD Rij_NoFsr_FF_FSR_UP_2(nb,nb);
  TMatrixD Rij_NoFsr_FF_FSR_UP_3(nb,nb);

  TMatrixD Rij_NoFsr_FF_FSR_DOWN_0(nb,nb);
  TMatrixD Rij_NoFsr_FF_FSR_DOWN_1(nb,nb);
  TMatrixD Rij_NoFsr_FF_FSR_DOWN_2(nb,nb);
  TMatrixD Rij_NoFsr_FF_FSR_DOWN_3(nb,nb);

  TMatrixD Rij_NoFsr_BB_FSR_UP_0(nb,nb);
  TMatrixD Rij_NoFsr_BB_FSR_UP_1(nb,nb);
  TMatrixD Rij_NoFsr_BB_FSR_UP_2(nb,nb);
  TMatrixD Rij_NoFsr_BB_FSR_UP_3(nb,nb);

  TMatrixD Rij_NoFsr_BB_FSR_DOWN_0(nb,nb);
  ...
  //fsr syst.
  TMatrixD Inv_Rij_NoFsr_FF_FSR_UP_0(nb,nb);
  TMatrixD Inv_Rij_NoFsr_FF_FSR_UP_1(nb,nb);
  TMatrixD Inv_Rij_NoFsr_FF_FSR_UP_2(nb,nb);
  TMatrixD Inv_Rij_NoFsr_FF_FSR_UP_3(nb,nb);

  TMatrixD Inv_Rij_NoFsr_FF_FSR_DOWN_0(nb,nb);
  TMatrixD Inv_Rij_NoFsr_FF_FSR_DOWN_1(nb,nb);
  TMatrixD Inv_Rij_NoFsr_FF_FSR_DOWN_2(nb,nb);
  TMatrixD Inv_Rij_NoFsr_FF_FSR_DOWN_3(nb,nb);

  TMatrixD Inv_Rij_NoFsr_BB_FSR_UP_0(nb,nb);
  TMatrixD Inv_Rij_NoFsr_BB_FSR_UP_1(nb,nb);
  TMatrixD Inv_Rij_NoFsr_BB_FSR_UP_2(nb,nb);
  TMatrixD Inv_Rij_NoFsr_BB_FSR_UP_3(nb,nb);

  TMatrixD Inv_Rij_NoFsr_BB_FSR_DOWN_0(nb,nb);
  ...
  //fsr syst
  TArrayD ff_NoFsr_FSR_UP_0(totdim);
  TArrayD ff_NoFsr_FSR_UP_1(totdim);
  TArrayD ff_NoFsr_FSR_UP_2(totdim);
  TArrayD ff_NoFsr_FSR_UP_3(totdim);

  TArrayD ff_NoFsr_FSR_DOWN_0(totdim);
  TArrayD ff_NoFsr_FSR_DOWN_1(totdim);
  TArrayD ff_NoFsr_FSR_DOWN_2(totdim);
  TArrayD ff_NoFsr_FSR_DOWN_3(totdim);

  TArrayD bb_NoFsr_FSR_UP_0(totdim);
  TArrayD bb_NoFsr_FSR_UP_1(totdim);
  TArrayD bb_NoFsr_FSR_UP_2(totdim);
  TArrayD bb_NoFsr_FSR_UP_3(totdim);

  TArrayD bb_NoFsr_FSR_DOWN_0(totdim);
  ...
  for (int i=0;i<nb;i++){
    for (int j=0;j<nb;j++){
      ...
      //fsr syst
      ff_NoFsr_FSR_UP_0[k] = Response_NoFsr_Forward_FSR_UP[0]->GetBinContent(i+1,j+1);
      ff_NoFsr_FSR_UP_1[k] = Response_NoFsr_Forward_FSR_UP[1]->GetBinContent(i+1,j+1);
      ff_NoFsr_FSR_UP_2[k] = Response_NoFsr_Forward_FSR_UP[2]->GetBinContent(i+1,j+1);
      ff_NoFsr_FSR_UP_3[k] = Response_NoFsr_Forward_FSR_UP[3]->GetBinContent(i+1,j+1);     

      ff_NoFsr_FSR_DOWN_0[k] = Response_NoFsr_Forward_FSR_DOWN[0]->GetBinContent(i+1,j+1);
      ff_NoFsr_FSR_DOWN_1[k] = Response_NoFsr_Forward_FSR_DOWN[1]->GetBinContent(i+1,j+1);
      ff_NoFsr_FSR_DOWN_2[k] = Response_NoFsr_Forward_FSR_DOWN[2]->GetBinContent(i+1,j+1);
      ff_NoFsr_FSR_DOWN_3[k] = Response_NoFsr_Forward_FSR_DOWN[3]->GetBinContent(i+1,j+1);    

      bb_NoFsr_FSR_UP_0[k] = Response_NoFsr_Backward_FSR_UP[0]->GetBinContent(i+1,j+1);
      bb_NoFsr_FSR_UP_1[k] = Response_NoFsr_Backward_FSR_UP[1]->GetBinContent(i+1,j+1);
      bb_NoFsr_FSR_UP_2[k] = Response_NoFsr_Backward_FSR_UP[2]->GetBinContent(i+1,j+1);
      bb_NoFsr_FSR_UP_3[k] = Response_NoFsr_Backward_FSR_UP[3]->GetBinContent(i+1,j+1);     

      bb_NoFsr_FSR_DOWN_0[k] = Response_NoFsr_Backward_FSR_DOWN[0]->GetBinContent(i+1,j+1);
  ...
  //fsr syst.
  Rij_NoFsr_FF_FSR_UP_0.SetMatrixArray(ff_NoFsr_FSR_UP_0.GetArray());
  Rij_NoFsr_FF_FSR_UP_1.SetMatrixArray(ff_NoFsr_FSR_UP_1.GetArray());
  Rij_NoFsr_FF_FSR_UP_2.SetMatrixArray(ff_NoFsr_FSR_UP_2.GetArray());
  Rij_NoFsr_FF_FSR_UP_3.SetMatrixArray(ff_NoFsr_FSR_UP_3.GetArray());

  Rij_NoFsr_FF_FSR_DOWN_0.SetMatrixArray(ff_NoFsr_FSR_DOWN_0.GetArray());
  Rij_NoFsr_FF_FSR_DOWN_1.SetMatrixArray(ff_NoFsr_FSR_DOWN_1.GetArray());
  Rij_NoFsr_FF_FSR_DOWN_2.SetMatrixArray(ff_NoFsr_FSR_DOWN_2.GetArray());
  Rij_NoFsr_FF_FSR_DOWN_3.SetMatrixArray(ff_NoFsr_FSR_DOWN_3.GetArray());

  Rij_NoFsr_BB_FSR_UP_0.SetMatrixArray(bb_NoFsr_FSR_UP_0.GetArray());
  Rij_NoFsr_BB_FSR_UP_1.SetMatrixArray(bb_NoFsr_FSR_UP_1.GetArray());
  Rij_NoFsr_BB_FSR_UP_2.SetMatrixArray(bb_NoFsr_FSR_UP_2.GetArray());
  Rij_NoFsr_BB_FSR_UP_3.SetMatrixArray(bb_NoFsr_FSR_UP_3.GetArray());

  Rij_NoFsr_BB_FSR_DOWN_0.SetMatrixArray(bb_NoFsr_FSR_DOWN_0.GetArray());
  ...
  //fsr syst.
  Inv_Rij_NoFsr_FF_FSR_UP_0 = Rij_NoFsr_FF_FSR_UP_0.Invert();
  Inv_Rij_NoFsr_FF_FSR_UP_1 = Rij_NoFsr_FF_FSR_UP_1.Invert();
  Inv_Rij_NoFsr_FF_FSR_UP_2 = Rij_NoFsr_FF_FSR_UP_2.Invert();
  Inv_Rij_NoFsr_FF_FSR_UP_3 = Rij_NoFsr_FF_FSR_UP_3.Invert();

  Inv_Rij_NoFsr_FF_FSR_DOWN_0 = Rij_NoFsr_FF_FSR_DOWN_0.Invert();
  Inv_Rij_NoFsr_FF_FSR_DOWN_1 = Rij_NoFsr_FF_FSR_DOWN_1.Invert();
  Inv_Rij_NoFsr_FF_FSR_DOWN_2 = Rij_NoFsr_FF_FSR_DOWN_2.Invert();
  Inv_Rij_NoFsr_FF_FSR_DOWN_3 = Rij_NoFsr_FF_FSR_DOWN_3.Invert();

  Inv_Rij_NoFsr_BB_FSR_UP_0 = Rij_NoFsr_BB_FSR_UP_0.Invert();
  Inv_Rij_NoFsr_BB_FSR_UP_1 = Rij_NoFsr_BB_FSR_UP_1.Invert();
  Inv_Rij_NoFsr_BB_FSR_UP_2 = Rij_NoFsr_BB_FSR_UP_2.Invert();
  Inv_Rij_NoFsr_BB_FSR_UP_3 = Rij_NoFsr_BB_FSR_UP_3.Invert();

  Inv_Rij_NoFsr_BB_FSR_DOWN_0 = Rij_NoFsr_BB_FSR_DOWN_0.Invert();
  ...
  //fsr syst.
  const double *inv_ff_NoFsr_FSR_UP_0 = Inv_Rij_NoFsr_FF_FSR_UP_0.GetMatrixArray();
  const double *inv_ff_NoFsr_FSR_UP_1 = Inv_Rij_NoFsr_FF_FSR_UP_1.GetMatrixArray();
  const double *inv_ff_NoFsr_FSR_UP_2 = Inv_Rij_NoFsr_FF_FSR_UP_2.GetMatrixArray();
  const double *inv_ff_NoFsr_FSR_UP_3 = Inv_Rij_NoFsr_FF_FSR_UP_3.GetMatrixArray();

  const double *inv_ff_NoFsr_FSR_DOWN_0 = Inv_Rij_NoFsr_FF_FSR_DOWN_0.GetMatrixArray();
  const double *inv_ff_NoFsr_FSR_DOWN_1 = Inv_Rij_NoFsr_FF_FSR_DOWN_1.GetMatrixArray();
  const double *inv_ff_NoFsr_FSR_DOWN_2 = Inv_Rij_NoFsr_FF_FSR_DOWN_2.GetMatrixArray();
  const double *inv_ff_NoFsr_FSR_DOWN_3 = Inv_Rij_NoFsr_FF_FSR_DOWN_3.GetMatrixArray();

  const double *inv_bb_NoFsr_FSR_UP_0 = Inv_Rij_NoFsr_BB_FSR_UP_0.GetMatrixArray();
  const double *inv_bb_NoFsr_FSR_UP_1 = Inv_Rij_NoFsr_BB_FSR_UP_1.GetMatrixArray();
  const double *inv_bb_NoFsr_FSR_UP_2 = Inv_Rij_NoFsr_BB_FSR_UP_2.GetMatrixArray();
  const double *inv_bb_NoFsr_FSR_UP_3 = Inv_Rij_NoFsr_BB_FSR_UP_3.GetMatrixArray();

  const double *inv_bb_NoFsr_FSR_DOWN_0 = Inv_Rij_NoFsr_BB_FSR_DOWN_0.GetMatrixArray();
  ...
  for (int i=0;i<nb;i++){
	...
	if (test_bb_NoFsr_3[k] != 1) cout<<"diagonal for the inv x resp for backward fsr matrix  "<< i<<"   "<<j<<"   "<<test_bb_NoFsr_3[k]<<endl;
      ...
      Inv_Response_UnDetector_Forward[0]->SetBinContent(i+1,j+1,inv_ff_0[k]);
      Inv_Response_UnDetector_Forward[1]->SetBinContent(i+1,j+1,inv_ff_1[k]);
      Inv_Response_UnDetector_Forward[2]->SetBinContent(i+1,j+1,inv_ff_2[k]);
      Inv_Response_UnDetector_Forward[3]->SetBinContent(i+1,j+1,inv_ff_3[k]);

      Inv_Response_NoFsr_Forward[0]->SetBinContent(i+1,j+1,inv_ff_NoFsr_0[k]);
      Inv_Response_NoFsr_Forward[1]->SetBinContent(i+1,j+1,inv_ff_NoFsr_1[k]);
      Inv_Response_NoFsr_Forward[2]->SetBinContent(i+1,j+1,inv_ff_NoFsr_2[k]);
      Inv_Response_NoFsr_Forward[3]->SetBinContent(i+1,j+1,inv_ff_NoFsr_3[k]);

      Inv_Response_UnDetector_Backward[0]->SetBinContent(i+1,j+1,inv_bb_0[k]);
      Inv_Response_UnDetector_Backward[1]->SetBinContent(i+1,j+1,inv_bb_1[k]);
      Inv_Response_UnDetector_Backward[2]->SetBinContent(i+1,j+1,inv_bb_2[k]);
      Inv_Response_UnDetector_Backward[3]->SetBinContent(i+1,j+1,inv_bb_3[k]);

      Inv_Response_NoFsr_Backward[0]->SetBinContent(i+1,j+1,inv_bb_NoFsr_0[k]);
      Inv_Response_NoFsr_Backward[1]->SetBinContent(i+1,j+1,inv_bb_NoFsr_1[k]);
      Inv_Response_NoFsr_Backward[2]->SetBinContent(i+1,j+1,inv_bb_NoFsr_2[k]);
      Inv_Response_NoFsr_Backward[3]->SetBinContent(i+1,j+1,inv_bb_NoFsr_3[k]);

      //fsr syst.
      Inv_Response_NoFsr_Forward_FSR_UP[0]->SetBinContent(i+1,j+1,inv_ff_NoFsr_FSR_UP_0[k]);
      Inv_Response_NoFsr_Forward_FSR_UP[1]->SetBinContent(i+1,j+1,inv_ff_NoFsr_FSR_UP_1[k]);
      Inv_Response_NoFsr_Forward_FSR_UP[2]->SetBinContent(i+1,j+1,inv_ff_NoFsr_FSR_UP_2[k]);
      Inv_Response_NoFsr_Forward_FSR_UP[3]->SetBinContent(i+1,j+1,inv_ff_NoFsr_FSR_UP_3[k]);

      Inv_Response_NoFsr_Forward_FSR_DOWN[0]->SetBinContent(i+1,j+1,inv_ff_NoFsr_FSR_DOWN_0[k]);
      Inv_Response_NoFsr_Forward_FSR_DOWN[1]->SetBinContent(i+1,j+1,inv_ff_NoFsr_FSR_DOWN_1[k]);
      Inv_Response_NoFsr_Forward_FSR_DOWN[2]->SetBinContent(i+1,j+1,inv_ff_NoFsr_FSR_DOWN_2[k]);
      Inv_Response_NoFsr_Forward_FSR_DOWN[3]->SetBinContent(i+1,j+1,inv_ff_NoFsr_FSR_DOWN_3[k]);

      Inv_Response_NoFsr_Backward_FSR_UP[0]->SetBinContent(i+1,j+1,inv_bb_NoFsr_FSR_UP_0[k]);
      Inv_Response_NoFsr_Backward_FSR_UP[1]->SetBinContent(i+1,j+1,inv_bb_NoFsr_FSR_UP_1[k]);
      Inv_Response_NoFsr_Backward_FSR_UP[2]->SetBinContent(i+1,j+1,inv_bb_NoFsr_FSR_UP_2[k]);
      Inv_Response_NoFsr_Backward_FSR_UP[3]->SetBinContent(i+1,j+1,inv_bb_NoFsr_FSR_UP_3[k]);

      Inv_Response_NoFsr_Backward_FSR_DOWN[0]->SetBinContent(i+1,j+1,inv_bb_NoFsr_FSR_DOWN_0[k]);
    ...
    for (int j=0;j<nb_Y;j++){
      ...
      file_cov->WriteTObject(hMeasCos_M_Y_Backward_FSR_UP[j],name_h);
      ...
      file_cov->WriteTObject(hUnDetectorCos_M_Y_Backward[j],name_h);

      sprintf(name_h,"undetector_forward_FSR_UP_%i",j);
      file_cov->WriteTObject(hUnDetectorCos_M_Y_Forward_FSR_UP[j],name_h);
      sprintf(name_h,"undetector_backward_FSR_UP_%i",j);
      file_cov->WriteTObject(hUnDetectorCos_M_Y_Backward_FSR_UP[j],name_h);

      sprintf(name_h,"undetector_forward_FSR_DOWN_%i",j);
      file_cov->WriteTObject(hUnDetectorCos_M_Y_Forward_FSR_DOWN[j],name_h);
      sprintf(name_h,"undetector_backward_FSR_DOWN_%i",j);
      file_cov->WriteTObject(hUnDetectorCos_M_Y_Backward_FSR_DOWN[j],name_h);

      sprintf(name_h,"truth_no_fsr_forward_%i",j);
      file_cov->WriteTObject(hNoFsrTruthCos_M_Y_Forward[j],name_h);
      sprintf(name_h,"truth_no_fsr_backward_%i",j);
      file_cov->WriteTObject(hNoFsrTruthCos_M_Y_Backward[j],name_h);

      sprintf(name_h,"response_undetector_forward_%i",j);
      file_cov->WriteTObject(Response_UnDetector_Forward[j],name_h);
      sprintf(name_h,"response_undetector_backward_%i",j);
      file_cov->WriteTObject(Response_UnDetector_Backward[j],name_h);

      sprintf(name_h,"response_undetector_forwardbackward_%i",j);
      file_cov->WriteTObject(Response_UnDetector_ForwardBackward[j],name_h);
      sprintf(name_h,"response_undetector_backwardforward_%i",j);
      file_cov->WriteTObject(Response_UnDetector_BackwardForward[j],name_h);

      sprintf(name_h,"response_undetector_forward_no_norm_%i",j);
      file_cov->WriteTObject(Response_UnDetector_Forward_NO_NORM[j],name_h);
      sprintf(name_h,"response_undetector_backward_no_norm_%i",j);
      file_cov->WriteTObject(Response_UnDetector_Backward_NO_NORM[j],name_h);

      sprintf(name_h,"response_undetector_forwardbackward_no_norm_%i",j);
      file_cov->WriteTObject(Response_UnDetector_ForwardBackward_NO_NORM[j],name_h);
      sprintf(name_h,"response_undetector_backwardforward_no_norm_%i",j);
      file_cov->WriteTObject(Response_UnDetector_BackwardForward_NO_NORM[j],name_h);

      sprintf(name_h,"response_nofsr_forward_%i",j);
      file_cov->WriteTObject(Response_NoFsr_Forward[j],name_h);
      sprintf(name_h,"response_nofsr_backward_%i",j);
      file_cov->WriteTObject(Response_NoFsr_Backward[j],name_h);
      // fsr syst.
      sprintf(name_h,"response_nofsr_forward_FSR_UP_%i",j);
      file_cov->WriteTObject(Response_NoFsr_Forward_FSR_UP[j],name_h);
      sprintf(name_h,"response_nofsr_backward_FSR_UP_%i",j);
      file_cov->WriteTObject(Response_NoFsr_Backward_FSR_UP[j],name_h);
      ...
      file_cov->WriteTObject(Inv_Response_UnDetector_Backward[j],name_h);

      sprintf(name_h,"inv_response_nofsr_forward_%i",j);
      file_cov->WriteTObject(Inv_Response_NoFsr_Forward[j],name_h);
      sprintf(name_h,"inv_response_nofsr_backward_%i",j);
      file_cov->WriteTObject(Inv_Response_NoFsr_Backward[j],name_h);

      //fsr syst.
      sprintf(name_h,"inv_response_nofsr_forward_FSR_UP_%i",j);
      file_cov->WriteTObject(Inv_Response_NoFsr_Forward_FSR_UP[j],name_h);
      sprintf(name_h,"inv_response_nofsr_backward_FSR_UP_%i",j);
      file_cov->WriteTObject(Inv_Response_NoFsr_Backward_FSR_UP[j],name_h);
