...
void tree1r()
{
 ...
 // TFile *file_cov = new TFile("MUON_MC_Meas_NoFsr_ForClosure.root","RECREATE"); float r_test = 0.5; 
  TFile *file_cov = new TFile("MUON_MC_Meas_UnDetector_NoFsr_DYbins_Updated_Zexpansion.root","RECREATE"); float r_test = 1.0;
  file_cov->cd(); 
  ...
  */
  ...
  TChain myTree("demo/MuonTree"); 
  myTree.Add("/data1/efe/ntuples/nhan/sin2thW_weakmodes/Zexpansion/newmuons.root");
...
////////////////////////////////////////
  ...
  for (int j=0;j<nb_Y;j++){
    ...
    Align_Zexpansion_hMeasCos_M_Y_Forward[j] = new TH1D(name_h,name_h,nb, xAxis_AFB);
    sprintf(name_h,"hMeasCos_Backward_Y_%i_",j);
    Align_Zexpansion_hMeasCos_M_Y_Backward[j] = new TH1D(name_h,name_h,nb, xAxis_AFB);
    ...
    sprintf(name_h,"Align_Zexpansion_Inv_Response_UnDetector_Forward_Y_%i_",j);
    Align_Zexpansion_Inv_Response_UnDetector_Forward[j] = new TH2D(name_h,name_h,nb, xAxis_AFB,nb, xAxis_AFB);
    sprintf(name_h,"Align_Zexpansion_Inv_Response_UnDetector_Backward_Y_%i_",j);
    Align_Zexpansion_Inv_Response_UnDetector_Backward[j] = new TH2D(name_h,name_h,nb, xAxis_AFB,nb, xAxis_AFB);

    sprintf(name_h,"Align_Zexpansion_Inv_Response_NoFsr_Forward_Y_%i_",j);
    Align_Zexpansion_Inv_Response_NoFsr_Forward[j] = new TH2D(name_h,name_h,nb, xAxis_AFB,nb, xAxis_AFB);
    sprintf(name_h,"Align_Zexpansion_Inv_Response_NoFsr_Backward_Y_%i_",j);
    Align_Zexpansion_Inv_Response_NoFsr_Backward[j] = new TH2D(name_h,name_h,nb, xAxis_AFB,nb, xAxis_AFB);
...
*/
    ...
    if (!realdata){
	  ...
	  if (fabs(Ymuon) > Y_bin_limits[j] && fabs(Ymuon) < Y_bin_limits[j+1]){ 
	    if (costhetaCSreco__RECO > 0) Align_Zexpansion_hMeasCos_M_Y_Forward[j]->Fill(MZ__RECO);
	    if (costhetaCSreco__RECO < 0 && costhetaCSreco__RECO >= -1.) Align_Zexpansion_hMeasCos_M_Y_Backward[j]->Fill(MZ__RECO);
  ...
  for (int i=0;i<nb;i++){
	...
	if (test_bb_NoFsr_3[k] != 1) cout<<"diagonal for the inv x resp for backward fsr matrix  "<< i<<"   "<<j<<"   "<<test_bb_NoFsr_3[k]<<endl;
      ...
      Align_Zexpansion_Inv_Response_UnDetector_Forward[0]->SetBinContent(i+1,j+1,inv_ff_0[k]);
      Align_Zexpansion_Inv_Response_UnDetector_Forward[1]->SetBinContent(i+1,j+1,inv_ff_1[k]);
      Align_Zexpansion_Inv_Response_UnDetector_Forward[2]->SetBinContent(i+1,j+1,inv_ff_2[k]);
      Align_Zexpansion_Inv_Response_UnDetector_Forward[3]->SetBinContent(i+1,j+1,inv_ff_3[k]);

      Align_Zexpansion_Inv_Response_NoFsr_Forward[0]->SetBinContent(i+1,j+1,inv_ff_NoFsr_0[k]);
      Align_Zexpansion_Inv_Response_NoFsr_Forward[1]->SetBinContent(i+1,j+1,inv_ff_NoFsr_1[k]);
      Align_Zexpansion_Inv_Response_NoFsr_Forward[2]->SetBinContent(i+1,j+1,inv_ff_NoFsr_2[k]);
      Align_Zexpansion_Inv_Response_NoFsr_Forward[3]->SetBinContent(i+1,j+1,inv_ff_NoFsr_3[k]);

      Align_Zexpansion_Inv_Response_UnDetector_Backward[0]->SetBinContent(i+1,j+1,inv_bb_0[k]);
      Align_Zexpansion_Inv_Response_UnDetector_Backward[1]->SetBinContent(i+1,j+1,inv_bb_1[k]);
      Align_Zexpansion_Inv_Response_UnDetector_Backward[2]->SetBinContent(i+1,j+1,inv_bb_2[k]);
      Align_Zexpansion_Inv_Response_UnDetector_Backward[3]->SetBinContent(i+1,j+1,inv_bb_3[k]);

      Align_Zexpansion_Inv_Response_NoFsr_Backward[0]->SetBinContent(i+1,j+1,inv_bb_NoFsr_0[k]);
      Align_Zexpansion_Inv_Response_NoFsr_Backward[1]->SetBinContent(i+1,j+1,inv_bb_NoFsr_1[k]);
      Align_Zexpansion_Inv_Response_NoFsr_Backward[2]->SetBinContent(i+1,j+1,inv_bb_NoFsr_2[k]);
      Align_Zexpansion_Inv_Response_NoFsr_Backward[3]->SetBinContent(i+1,j+1,inv_bb_NoFsr_3[k]);
    }
  }
    ...
    for (int j=0;j<nb_Y;j++){
      ...
      file_cov->WriteTObject(Align_Zexpansion_hMeasCos_M_Y_Forward[j],name_h);
      sprintf(name_h,"Align_Zexpansion_MC_meas_Backward_%i",j);
      file_cov->WriteTObject(Align_Zexpansion_hMeasCos_M_Y_Backward[j],name_h);


      sprintf(name_h,"Align_Zexpansion_inv_response_undetector_forward_%i",j);
      file_cov->WriteTObject(Align_Zexpansion_Inv_Response_UnDetector_Forward[j],name_h);
      sprintf(name_h,"Align_Zexpansion_inv_response_undetector_backward_%i",j);
      file_cov->WriteTObject(Align_Zexpansion_Inv_Response_UnDetector_Backward[j],name_h);

      sprintf(name_h,"Align_Zexpansion_inv_response_nofsr_forward_%i",j);
      file_cov->WriteTObject(Align_Zexpansion_Inv_Response_NoFsr_Forward[j],name_h);
      sprintf(name_h,"Align_Zexpansion_inv_response_nofsr_backward_%i",j);
      file_cov->WriteTObject(Align_Zexpansion_Inv_Response_NoFsr_Backward[j],name_h);
    }
  //  file_cov->Write();
